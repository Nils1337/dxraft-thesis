...
Raft teilt die Zeit in \textit{Terms} auf, die fortlaufend nummeriert werden. Jeder Term beginnt mit der Wahl eines Leaders. Sobald ein Leader gewählt wurde, beginnt der normale Betrieb. Falls in einem Term kein Leader gewählt wird, z.B. weil die Stimmen auf mehrere Server aufgeteilt sind, dann endet ein Term ohne einen Leader. Ein Server beginnt einen neuen Term, falls er denkt, dass der Leader ist ausgefallen oder die Wahl zu keinem Ergebnis gekommen ist. Abbildung \ref{fig:terms} zeigt ein Beispiel wie Terms ablaufen können. \\
